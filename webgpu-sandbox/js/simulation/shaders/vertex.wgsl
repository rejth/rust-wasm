...
struct VertexInput {
  ...
  @builtin(instance_index) instance: u32, // Instance is the index of the geometry we draw. If we draw 6 rectangles, instances will be 0, 1, 2, 3, 4, 5.
};

struct VertexOutput {
  @builtin(position) position: vec4f, // Position of the vertex in the clip space after all transformations are applied.
...
fn main(input: VertexInput) -> VertexOutput {
  ...
  */
  ...
  return VertexOutput(vec4f(grid_position, 0, 1), cell);
}
